  ...
  function YYResumeContent() {
    ...
    self.__yyStylesReady = new Promise(function (resolve) {
      ...
      function done() {
        ...
        // #region agent log
        fetch('http://127.0.0.1:7399/ingest/3a7f2c1e-debug',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'nav-flash'},body:JSON.stringify({sessionId:'nav-flash',runId:'post-fix',hypothesisId:'B',location:'yy-resume.js:css-ready',message:'yy-resume styles ready; pending cleared',data:{href:sheet.href},timestamp:Date.now()})}).catch(function(){});
        // #endregion
        resolve();
      }
    ...
    // #region agent log
    fetch('http://127.0.0.1:7399/ingest/3a7f2c1e-debug',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'nav-flash'},body:JSON.stringify({sessionId:'nav-flash',runId:'post-fix',hypothesisId:'B',location:'yy-resume.js:ctor',message:'CE constructed with pending until CSS',data:{href:sheet.href,pending:true},timestamp:Date.now()})}).catch(function(){});
    // #endregion
