/* ============================================================================
   yy-chrome.js — the shared chrome layer: one capsule nav (Work / About /
   Resume panels) + one footer credit, on every page.

   Pages are keyed by ROUTE ('/', '/work/larkdesign', …). The Work / About /
   Resume panels render from the JSON block BaseLayout.astro embeds as
   #yy-content (built from src/content), so copy is edited in one place.

   HOW IT LOADS
   Deferred script from BaseLayout.astro. It stamps `html.yy-chrome`, loads
   yy-chrome.css and mounts once the body is parsed.

   FAILING SAFE
   404, parse error, CSP block, mount throw → `html.yy-chrome` is removed and
   the page renders without chrome. Rollback is deleting one <script> line.
...
(function () {
  ...
  /* Normalised route: '/', '/work', '/work/larkdesign'. Tolerates a trailing
     slash, /index.html, and the old .html filenames (redirected by Vercel). */
  ...
  var DARK_NAV_PAGES = {
    '/work/ai-driven-product-design': true
  };
  ...
  var FLOW_PALETTES = {
    '/work/larkdesign':               { base: '#2a73e2', alt: '#9aa3ad', idle: '#e6e8eb' },
    '/work/mckinseyecommerce':        { base: '#e03400', alt: '#a89a94', idle: '#ebe7e4' },
    '/work/ai-driven-product-design': { base: '#d4c8ff', alt: '#e8e6f2', idle: '#c5c0d4' },
    '/work/mifinance':                { base: '#e8710a', alt: '#9a958e', idle: '#ebe8e4' },
    '/work/cummins-digitalization':   { base: '#980000', alt: '#9a9290', idle: '#ebe7e6' },
    '/work/alzheimerdisease':         { base: '#8eb0f0', alt: '#d8dce4', idle: '#b0b4bc' },
    '/work/tiktok-research':          { base: '#3d5a6c', alt: '#9aa3ad', idle: '#e6e8eb' }
  };
  ...
  /* Skip-link target: <main> when present, else the first content child of
     <body>. Never overwrites an existing id. */
  ...
  function mount() {
    ...
    var navHTML =
      ...
      '<nav class="cap" aria-label="Main">' +
        '<a class="brand" href="/" aria-label="Yanice Yang home"' +
    ...
    /* ---- footer — credit only, appended to <body> ---- */
    var footHTML =
      '<footer class="ft">' +
  ...
  function go() {
    try {
      ...
      if (window.console) console.error('[yy-chrome] mount failed:', e);
    }
  }
